...
1. User
    ...
    - Usuarios del sistema: directores, laboratoristas, admin.

    - Campos: id, nombre, email, rol, contraseña, etc.

    - Relaciones:

        - 1:N con SolicitudEnsayo (creador)

        - 1:N con Ensayo (laboratorista asignado)

        - 1:N con Informe (generadoPor)

2. Obra

    - Representa una obra/construcción.

    - Un director puede estar en varias obras.

    - Un laboratorio puede trabajar con cualquier obra.

    - Relaciones:

        - N:M con User (solo directores)

        - 1:N con SolicitudEnsayo

        - 1:N con Ensayo

        - 1:N con Equipo (equipo asignado a una obra)

3. SolicitudEnsayo

    - Pedido de un ensayo realizado por un director.

    - Campos: tipoDeEnsayo, descripcion, obra, creadoPor, estado.
...
4. Ensayo
    ...
    - Ensayo ya en proceso o realizado.

    - Deriva directamente de una solicitud.

    - Campos: datos, estado, gráficos (si los agregamos), laboratorista.

    - Relaciones:

        - 1:1 con SolicitudEnsayo

        - N:1 con Obra

        - N:1 con User (laboratorista)

        - 1:1 con Informe

        - N:1 con Equipo (opcional, si se usa un equipo específico)

5. Informe

    - Un PDF generado a partir del ensayo.

    - Campos: archivoUrl, nombreArchivo, version, estado.

    - Relaciones:

        - 1:1 con Ensayo

        - N:1 con User (laboratorista que lo generó)

6. Equipo

    - Equipos de laboratorio: prensa, balanza, compresor, etc.

    - Globales (no dependen de una obra).

    - Pero se pueden asignar a una obra temporalmente.

    - Campos: nombre, tipo, estado, obraActual.

    - Relaciones:

        - N:1 con Obra (equipo actualmente asignado)

        - 1:N con HistorialEquipo (movimientos del equipo)

        - 1:N con Ensayo si querés registrar qué equipo realizó el ensayo (opcional)
...
7. HistorialEquipo
    ...
    - De laboratorio → a Obra A

    - De Obra A → Obra B

    - Campos: fecha, accion, origen, destino.
